...
proc	 	= np.array([4,3,2])		 	# processor layout
...
dens	 	= np.array([13900])			# Particle densities
cres	 	= np.array([0.14])		 	# Coefficients of restitution
...
filestr         = "re20au" 	  			# Base filename string for input files
E		= 7.900e9					# LIGGGHTS parameter Young's modulus for all particles
nu		= 0.42					# LIGGGHTS parameter Poisson's ratio for all particles
steps     	= 25000000      			# Number of simulation steps after initialization
shearStr        = 100.0					# Shear strain rate to apply for shear simulations
lens            = 1.5e-6                               # representative length scale, bounding box length/largest size. Can be used to control #particles for a fixed volume
lebc_band       = 0.15
lebc_latt       = 0
...
dtrelax         = 0.3                                  # time step relaxation (dt=dtrelax*dt_stable)
...
lebc.pbs_files(1,24,24,"","16:00:00","normal","rxl","lmp_auto","shear", "module use -a /home/rxl/local/modules\nmodule purge\nmodule load openmpi\nmodule load vtk\n")
