...
distribution    = 'curtis_literature'     				# 'linear' or 'log' for alpha distribution
npoints   	= 8					# Number of volume fractions, must be > 1
#nparticles      = 5000					# Total number of particles to use
filestr         = "curl_5" 	  			# Base filename string for input files
E		= 8.7e9					# LIGGGHTS parameter Young's modulus for all particles
nu		= 0.3					# LIGGGHTS parameter Poisson's ratio for all particles
steps     	= 24000000      			# Number of simulation steps after initialization
shearStr        = 100.0					# Shear strain rate to apply for shear simulations
lens            = 1.2e-4                               # representative length scale, bounding box length/largest size. Can be used to control #particles for a fixed volume
lebc_band       = 0.15
lebc_latt       = 0
...
dtrelax         = 0.4                                   # time step relaxation (dt=dtrelax*dt_stable)
...
proc	 	= np.array([3,2,1])		 	# processor layout
ptype	 	= np.array(["curl_5.particle"])        	# Particle type, blank is sphere, file names means multisphere
filestr         = "curl_5"	  			# Base filename string for input files
lebc.set_input(nsph,proc,ptype,nspecies,diam,dens,cres,amin,amax,asmax,distribution,npoints,lens,lebc_band,lebc_latt,filestr,E,nu,steps,shearStr,gtemp,dtrelax,nbins_x, "true")
# Check for errors in input parameters
lebc.check_input()
...
#     proc	 	= np.array([3,2,1])		 	# processor layout
#     ptype	 	= np.array(["curl_"+str(i) + ".particle"])        	# Particle type, blank is sphere, file names means multisphere
#     filestr         = "curl_" + str(i) 	  			# Base filename string for input files
#     lebc.set_input(nsph,proc,ptype,nspecies,diam,dens,cres,amin,amax,asmax,distribution,npoints,lens,lebc_band,lebc_latt,filestr,E,nu,steps,shearStr,gtemp,dtrelax,nbins_x, "true")
#     # Check for errors in input parameters
#     lebc.check_input()
#     # Calculations to set parameters specified in inputs
...
#     lebc.set_init_params(rinit_xmax,rinit_reset,rinit_th,rinit_ns)
    ...
    #    if j > 3:
        #    proc	 	= np.array([4,4,2])	
        #    lebc.set_proc(proc)
        #lebc.shear_sim_files_specific(j)
        #lebc.pbs_files_specific(j,1,proc[0]*proc[1]*proc[2],proc[0]*proc[1]*proc[2],"","16:00:00","normal","liz.suehr","lmp_auto","shear", "module load /home/liz.suehr/modules/openmpi/1.10.7\nmodule load /home/liz.suehr/modules/vtk/8.2.0\n")
