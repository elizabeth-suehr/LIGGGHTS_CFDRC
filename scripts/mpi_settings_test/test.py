...
#Name of test is the directory file name, passed from validation.py
# test_name = ["multisphere13.txt"]
# files = ["multisphere13"]

# particles_num_spheres = []

# for file in files:
#     try:
#         with open(file) as f:
#             particles_num_spheres.append(sum(1 for line in f if line.strip()))
#     except:
#         print("No particle data found for ",file)

# #If no files provided assumed to be just spheres
# if len(particles_num_spheres) == 0:
#     particles_num_spheres.append(1)
#     files = []
#     files.append("")
...
diam	 	= np.array([0.0001])      		# Particle diameters, can be anything here for multisphere as it is computed before use
...
npoints   	= 2					# Number of volume fractions, must be > 1
#nparticles      = 5000					# Total number of particles to use
filestr         = "multisphere_13" 	  			# Base filename string for input files
E		= 8.7e9					# LIGGGHTS parameter Young's modulus for all particles
nu		= 0.3					# LIGGGHTS parameter Poisson's ratio for all particles
steps     	= 'timetest'      			# Number of simulation steps after initialization
shearStr        = 100.0					# Shear strain rate to apply for shear simulations
lens            = 0.00014*2.0 * 1.2                       # representative length scale, bounding box length/largest size. Can be used to control #particles for a fixed volume
lebc_band       = 0.15
lebc_latt       = 0
...
dtrelax         = 0.1                                   # time step relaxation (dt=dtrelax*dt_stable)
...
dif_particles = ["multisphere13","multisphere43","multisphere93","multisphere163"]
dif_num = [13,43,93,163]
for (dif_p, num) in zip(dif_particles,dif_num):
    ptype	 	= np.array([dif_p + ".txt"]) 
    nsph            = np.array([num]) 
    for i in range(1,3):
        mpi_setting_string = "_MPI"+str(i*2) + str(i*2) + str(i)
        proc	 	= np.array([i*2,i*2,i]) 	# processor layout
        filestr         = dif_p + mpi_setting_string	  			# Base filename string for input files
        if i==2:
            mpi_setting_string = "_MPI"+str(3) + str(3) + str(2)
            proc	 	= np.array([3,3,2]) 	# processor layout
            filestr         = dif_p + mpi_setting_string	  			# Base filename string for input files
        ...
        ##################################################################
        nodes = 1
        total_cores = proc[0]*proc[1]*proc[2]

        if total_cores > 32:
            nodes = total_cores // 28
            if total_cores % 28 != 0:
                nodes +=1
            total_cores = 28
        ...
        lebc.pbs_files(nodes,total_cores,total_cores,"","16:00:00","normal","liz.suehr","lmp_auto","shear", "module load /home/liz.suehr/modules/openmpi/1.10.7\nmodule load /home/liz.suehr/modules/vtk/8.2.0\n")
        lebc.del_globals()



#Name of test is the directory file name, passed from validation.py
test_name = ["single_sphere"]
files = []
...
# User inputs for LIGGGHTS simulations
nspecies        = 1					# Number of species to use
proc	 	= np.array([3,2,1])		 	# processor layout
ptype	 	= np.array(files)        	# Particle type, blank is sphere, file names means multisphere
# ptype	 	= np.array(["","","particle"])      	# Particle type, blank is sphere, file names means multisphere
nsph            = np.array(particles_num_spheres)                         # number of spheres for body, 1 for pure sphere (not body)
diam	 	= np.array([0.0001])      		# Particle diameters, can be anything here for multisphere as it is computed before use
...
npoints   	= 2					# Number of volume fractions, must be > 1
#nparticles      = 5000					# Total number of particles to use
filestr         = "s_sphere" 	  			# Base filename string for input files
E		= 8.7e9					# LIGGGHTS parameter Young's modulus for all particles
nu		= 0.3					# LIGGGHTS parameter Poisson's ratio for all particles
steps     	= 'timetest'       			# Number of simulation steps after initialization
shearStr        = 100.0					# Shear strain rate to apply for shear simulations
lens            = 1.2e-4  * 1.2                               # representative length scale, bounding box length/largest size. Can be used to control #particles for a fixed volume
lebc_band       = 0.15
lebc_latt       = 0
gtemp           = 1.0                                   # granular temperature, used for cooling
rinit_xmax      = 1e-4                                  # initializer, maximum limit distance per timestep
...
for i in range(1,3):
    mpi_setting_string = "_MPI"+str(i*2) + str(i*2) + str(i)
    proc	 	= np.array([i*2,i*2,i]) 	# processor layout
    filestr         = "s_sphere" + mpi_setting_string	  			# Base filename string for input files
    if i==2:
        mpi_setting_string = "_MPI"+str(3) + str(3) + str(2)
        proc	 	= np.array([3,3,2]) 	# processor layout
        filestr         = "s_sphere" + mpi_setting_string	  			# Base filename string for input files
