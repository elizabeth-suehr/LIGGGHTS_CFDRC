...
nbins_x = 0
isDimensional = "true"
density2        = 2500.00                                  # density type 2 
...
def shear_sim_files_specific(j):
  ...
  for i in range(0,nspecies):
    ...
    fout.write('group             nve_group2 region domain\n')
    if (int(pmode[i])):
      # print(i,pmode[i],dens[i],nsph[i],ptype[i])
      fout.write('fix               pts{0} nve_group{0} particletemplate/multisphere 123457 atom_type {0} volume_limit 1e-18 density constant {1:.3f} nspheres {2} ntry 10000000 spheres_different_types file {3} scale 1.0 type {4} density2 {5}\n'.format(i+1,dens[i],nsph[i],ptype[i],ims,density2))
      fout.write('fix               pts2 nve_group2 particletemplate/sphere 123457 atom_type 2 volume_limit 1e-18 density constant 2500.00 radius constant 0.0005\n')
      ims=ims+i
    else:
