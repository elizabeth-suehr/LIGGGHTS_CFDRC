...
void Verlet::setup()
{
  ...
  if (comm->me == 0 && screen)
    fprintf(screen, "Setting up run at %s\n", ctime(&curtime));
  if (comm->me == 0 && logfile)
    fprintf(logfile, "Setting up run at %s\n", ctime(&curtime));
  ...
  if (domain->lebc && domain->lebc_displacement == 0.0)
    domain->lebc_displacement += 0.0000000000001 * domain->xprd; // werid fix i know....
...
void Verlet::run(int n)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    if (n_pre_initial_integrate)
      modify->pre_initial_integrate();
    ...
    if (n_pre_final_integrate)
      modify->pre_final_integrate();
