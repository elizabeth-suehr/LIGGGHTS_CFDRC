...
FixRigid::FixRigid(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),
                                                        random(NULL)
{
  // not working correctly for 2d
  if (domain->dimension == 2)
    error->warning(FLERR, "Fix rigid should not be used for 2d simulations - inertia is for 3d systems");

  if (narg > 3 && strcmp(arg[3], "pseudo") == 0)
    return;
  ...
  if (nbody == 0)
    error->all(FLERR, "No rigid bodies defined");
  ...
  for (i = 0; i < nbody; i++)
  {
    ...
    if (domain->dimension == 2)
      fflag[i][2] = tflag[i][0] = tflag[i][1] = 0.0;
  }
  ...
  for (ibody = 0; ibody < nbody; ibody++)
    if (nrigid[ibody] <= 1)
      error->all(FLERR, "One or zero atoms in rigid body");
  ...
  {
    if (screen)
      fprintf(screen, "%d rigid bodies with %d atoms\n", nbody, nsum);
    if (logfile)
      fprintf(logfile, "%d rigid bodies with %d atoms\n", nbody, nsum);
  }
...
int FixRigid::dof(int tgroup)
{
  ...
  for (int ibody = 0; ibody < nbody; ibody++)
  {
    if (nall[ibody] + mall[ibody] > 0 &&
        nall[ibody] + mall[ibody] != nrigid[ibody])
      flag = 1;
  }
  if (flag && me == 0)
...
void FixRigid::set_xv()
{
  ...
  if (extended)
  {
    ...
    if (avec_ellipsoid)
      ebonus = avec_ellipsoid->bonus;
    AtomVecLine::Bonus *lbonus = NULL;
    if (avec_line)
      lbonus = avec_line->bonus;
    ...
    for (int i = 0; i < nlocal; i++)
    {
      ...
      {
        if (quat[ibody][3] >= 0.0)
          theta_body = 2.0 * acos(quat[ibody][0]);
        else
          theta_body = -2.0 * acos(quat[ibody][0]);
...
void FixRigid::setup_bodies_static()
{
  ...
  for (ibody = 0; ibody < nbody; ibody++)
  {
    ...
    if (inertia[ibody][0] < EPSILON * max)
      inertia[ibody][0] = 0.0;
    if (inertia[ibody][1] < EPSILON * max)
      inertia[ibody][1] = 0.0;
    if (inertia[ibody][2] < EPSILON * max)
      inertia[ibody][2] = 0.0;
  ...
  for (i = 0; i < nlocal; i++)
  {
    ...
    if (extended)
    {
      ...
      else if (eflags[i] & LINE)
      {
        ...
        while (orient[i][0] <= MINUSPI)
          orient[i][0] += TWOPI;
        while (orient[i][0] > MY_PI)
          orient[i][0] -= TWOPI;
        if (orientflag == 4)
          orient[i][1] = orient[i][2] = orient[i][3] = 0.0;
      }
...
void FixRigid::readfile(int which, double *vec, double **array, int *inbody)
{
  ...
  if (me == 0)
  {
    ...
    while (1)
    {
      ...
      if (eof == NULL)
        error->one(FLERR, "Unexpected end of fix rigid file");
  ...
  if (nlines == 0)
    error->all(FLERR, "Fix rigid file has no lines");
  ...
  while (nread < nlines)
  {
    ...
    if (eofflag)
      error->all(FLERR, "Unexpected end of fix rigid file");
...
double FixRigid::memory_usage()
{
  ...
  if (extended)
  {
    ...
    if (orientflag)
      bytes = nmax * orientflag * sizeof(double);
    if (dorientflag)
      bytes = nmax * 3 * sizeof(double);
  }
  return bytes;
...
void FixRigid::grow_arrays(int nmax)
{
  ...
  if (extended)
  {
    ...
    if (orientflag)
      memory->grow(orient, nmax, orientflag, "rigid:orient");
    if (dorientflag)
      memory->grow(dorient, nmax, 3, "rigid:dorient");
  }
}
...
double FixRigid::compute_array(int i, int j)
{
  ...
  if (j < 12)
    return torque[i][j - 9];
  if (j == 12)
    return (imagebody[i] & IMGMASK) - IMGMAX;
  if (j == 13)
    return (imagebody[i] >> IMGBITS & IMGMASK) - IMGMAX;
