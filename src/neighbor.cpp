...
void Neighbor::init()
{
  ...
  if (respa)
  {
    ...
    if (cut_respa[0] - skin < 0)
      cut_middle_inside_sq = 0.0;
  }
  ...
  if (!same)
  {
      ...
      else if (requests[i]->fix || requests[i]->compute)
      {
        for (j = 0; j < nlist; j++)
        {
          if (requests[i]->half && requests[j]->pair &&
              requests[j]->skip == 0 && requests[j]->half)
            break;
          if (requests[i]->full && requests[j]->pair &&
              requests[j]->skip == 0 && requests[j]->full)
            break;
          if (requests[i]->gran && requests[j]->pair &&
              requests[j]->skip == 0 && requests[j]->gran)
            break;
          if (requests[i]->half && requests[j]->pair &&
              requests[j]->skip == 0 && requests[j]->respaouter)
            break;
        }
    ...
    {
      lists[i]->buildflag = 1;
      if (pair_build[i] == NULL)
        lists[i]->buildflag = 0;
      if (requests[i]->occasional)
        lists[i]->buildflag = 0;
      ...
      if (style == NSQ)
        lists[i]->stencilflag = 0;
      if (stencil_create[i] == NULL)
        lists[i]->stencilflag = 0;

      lists[i]->ghostflag = 0;
      if (requests[i]->ghost)
        lists[i]->ghostflag = 1;
      if (requests[i]->ghost && !requests[i]->occasional)
        anyghostlist = 1;
    }
    ...
    while (!done)
    {
      ...
      {
        NeighList *ptr = NULL;
        if (lists[blist[i]]->listfull)
          ptr = lists[blist[i]]->listfull;
        if (lists[blist[i]]->listcopy)
          ptr = lists[blist[i]]->listcopy;
        if (lists[blist[i]]->listskip)
          ptr = lists[blist[i]]->listskip;
        if (ptr == NULL)
          continue;
        for (m = 0; m < nlist; m++)
          if (ptr == lists[m])
  ...
  if (bond_off)
    bond_build = &Neighbor::bond_partial;
  else
    bond_build = &Neighbor::bond_all;

  if (angle_off)
    angle_build = &Neighbor::angle_partial;
  else
    angle_build = &Neighbor::angle_all;

  if (dihedral_off)
    dihedral_build = &Neighbor::dihedral_partial;
  else
    dihedral_build = &Neighbor::dihedral_all;

  if (improper_off)
    improper_build = &Neighbor::improper_partial;
  else
    improper_build = &Neighbor::improper_all;
...
void Neighbor::choose_build(int index, NeighRequest *rq)
{
  ...
  if (rq->omp == 0)
  {
    ...
    else if (rq->skip)
    {
      if (rq->gran && lists[index]->listgranhistory)
        ...
        pb = &Neighbor::skip_from_respa;
      else
        pb = &Neighbor::skip_from;
    }
        ...
        if (rq->newton == 0)
        {
          if (newton_pair == 0)
          {
            if (rq->ghost == 0)
              ...
              pb = &Neighbor::half_nsq_no_newton_ghost;
          }
          else if (newton_pair == 1)
        ...
        if (rq->newton == 0)
        {
          if (newton_pair == 0)
          {
            if (rq->ghost == 0)
              ...
              pb = &Neighbor::half_bin_no_newton_ghost;
          }
          else if (triclinic == 0)
          {
      ...
      else if (style == MULTI)
      {
        ...
        if (rq->newton == 0)
        {
          if (newton_pair == 0)
            ...
            pb = &Neighbor::half_multi_newton;
          else if (triclinic == 1)
            pb = &Neighbor::half_multi_newton_tri;
        }
        else if (rq->newton == 1)
        {
          if (triclinic == 0)
    ...
    else if (rq->full)
    {
      ...
      {
        if (rq->ghost == 0)
          pb = &Neighbor::full_nsq;
        else if (includegroup)
          error->all(FLERR,
                     "Neighbor include group not allowed with ghost neighbors");
        else
          pb = &Neighbor::full_nsq_ghost;
      }
      else if (style == BIN)
      {
        if (rq->ghost == 0)
          pb = &Neighbor::full_bin;
        else if (includegroup)
          error->all(FLERR,
                     "Neighbor include group not allowed with ghost neighbors");
        else
          pb = &Neighbor::full_bin_ghost;
      }
      else if (style == MULTI)
      {
        if (rq->ghost == 1)
    ...
    else if (rq->gran)
    {
      ...
      {
        if (newton_pair == 0)
          pb = &Neighbor::granular_nsq_no_newton;
        else if (newton_pair == 1)
          pb = &Neighbor::granular_nsq_newton;
      }
      else if (style == BIN)
      {
        if (newton_pair == 0)
          pb = &Neighbor::granular_bin_no_newton;
        else if (triclinic == 0)
          pb = &Neighbor::granular_bin_newton;
        else if (triclinic == 1)
      ...
      else if (style == MULTI)
      {
        if (0)
        {
          ...
          error->all(FLERR, "Neighbor multi not yet enabled for granular with triclinic");
        else if (1 == newton_pair)
          error->all(FLERR, "Neighbor multi not yet enabled for granular with newton on");
        else if (0 == newton_pair)
          pb = &Neighbor::granular_multi_no_newton;
        else
    ...
    else if (rq->respaouter)
    {
      ...
      {
        if (newton_pair == 0)
          pb = &Neighbor::respa_nsq_no_newton;
        else if (newton_pair == 1)
          pb = &Neighbor::respa_nsq_newton;
      }
      else if (style == BIN)
      {
        if (newton_pair == 0)
          pb = &Neighbor::respa_bin_no_newton;
        else if (triclinic == 0)
          pb = &Neighbor::respa_bin_newton;
        else if (triclinic == 1)
          pb = &Neighbor::respa_bin_newton_tri;
      }
      else if (style == MULTI)
  ...
  else
  {
    ...
    {
      if (newton_pair == 0)
        pb = &Neighbor::half_from_full_no_newton_omp;
      else if (newton_pair == 1)
        pb = &Neighbor::half_from_full_newton_omp;
    }
    else if (rq->half)
    {
      if (style == NSQ)
        ...
        if (rq->newton == 0)
        {
          if (newton_pair == 0)
          {
            if (rq->ghost == 0)
              ...
              pb = &Neighbor::half_nsq_no_newton_ghost_omp;
          }
          else if (newton_pair == 1)
      ...
      else if (style == BIN)
      {
        if (rq->newton == 0)
        {
          ...
          {
            if (rq->ghost == 0)
              pb = &Neighbor::half_bin_no_newton_omp;
            else if (includegroup)
              error->all(FLERR, "Neighbor include group not allowed "
                                "with ghost neighbors");
            else
              pb = &Neighbor::half_bin_no_newton_ghost_omp;
          }
          else if (triclinic == 0)
          {
      ...
      else if (style == MULTI)
      {
        ...
        {
          if (newton_pair == 0)
            pb = &Neighbor::half_multi_no_newton_omp;
          else if (triclinic == 0)
            pb = &Neighbor::half_multi_newton_omp;
          else if (triclinic == 1)
            pb = &Neighbor::half_multi_newton_tri_omp;
        }
        else if (rq->newton == 1)
        {
          if (triclinic == 0)
    ...
    else if (rq->full)
    {
      ...
      {
        if (rq->ghost == 0)
          pb = &Neighbor::full_nsq_omp;
        else if (includegroup)
          error->all(FLERR,
                     "Neighbor include group not allowed with ghost neighbors");
        else
          pb = &Neighbor::full_nsq_ghost_omp;
      }
      else if (style == BIN)
      {
        if (rq->ghost == 0)
          pb = &Neighbor::full_bin_omp;
        else if (includegroup)
          error->all(FLERR,
                     "Neighbor include group not allowed with ghost neighbors");
        else
          pb = &Neighbor::full_bin_ghost_omp;
      }
      else if (style == MULTI)
      {
        if (rq->ghost == 1)
    ...
    else if (rq->gran)
    {
      ...
      {
        if (newton_pair == 0)
          pb = &Neighbor::granular_nsq_no_newton_omp;
        else if (newton_pair == 1)
          pb = &Neighbor::granular_nsq_newton_omp;
      }
      else if (style == BIN)
      {
        if (newton_pair == 0)
          pb = &Neighbor::granular_bin_no_newton_omp;
        else if (triclinic == 0)
          pb = &Neighbor::granular_bin_newton_omp;
        else if (triclinic == 1)
          pb = &Neighbor::granular_bin_newton_tri_omp;
      }
      else if (style == MULTI)
        error->all(FLERR, "Neighbor multi not yet enabled for granular");
    }
    else if (rq->respaouter)
    {
      ...
      {
        if (newton_pair == 0)
          pb = &Neighbor::respa_nsq_no_newton_omp;
        else if (newton_pair == 1)
          pb = &Neighbor::respa_nsq_newton_omp;
      }
      else if (style == BIN)
      {
        if (newton_pair == 0)
          pb = &Neighbor::respa_bin_no_newton_omp;
        else if (triclinic == 0)
          pb = &Neighbor::respa_bin_newton_omp;
        else if (triclinic == 1)
          pb = &Neighbor::respa_bin_newton_tri_omp;
      }
      else if (style == MULTI)
        error->all(FLERR, "Neighbor multi not yet enabled for rRESPA");
    }
...
void Neighbor::choose_stencil(int index, NeighRequest *rq)
{
  ...
  if (rq->skip || rq->copy || rq->half_from_full)
    sc = NULL;
    ...
    if (style == BIN)
    {
      if (rq->newton == 0)
      {
        if (newton_pair == 0)
        {
          ...
          {
            if (rq->ghost)
              sc = &Neighbor::stencil_half_ghost_bin_2d_no_newton;
            else
          ...
          {
            if (rq->ghost)
              sc = &Neighbor::stencil_half_ghost_bin_3d_no_newton;
            else
  ...
  else if (rq->full)
  {
    if (style == BIN)
    {
      ...
      {
        if (rq->ghost)
          sc = &Neighbor::stencil_full_ghost_bin_2d;
        else
          sc = &Neighbor::stencil_full_bin_2d;
      }
      else if (dimension == 3)
      {
        if (rq->ghost)
          sc = &Neighbor::stencil_full_ghost_bin_3d;
        else
          sc = &Neighbor::stencil_full_bin_3d;
      }
    }
    else if (style == MULTI)
    {
      if (dimension == 2)
        sc = &Neighbor::stencil_full_multi_2d;
      else if (dimension == 3)
        sc = &Neighbor::stencil_full_multi_3d;
    }
  }
...
int Neighbor::decide()
{
  if (must_check)
  {
    ...
    if (output->restart_requested(update->ntimestep))
      return 1;
...
void Neighbor::build_one(int i)
{
  ...
  int flag = 0;
  if (dist_check && update->whichflag)
    flag = check_distance();
  if (flag && me == 0)
...
void Neighbor::setup_bins()
{
  ...
  if (sx * binsizex < cutneighmax)
    sx++;
  sy = static_cast<int>(cutneighmax * bininvy);
  if (sy * binsizey < cutneighmax)
    sy++;
  sz = static_cast<int>(cutneighmax * bininvz);
  if (sz * binsizez < cutneighmax)
    sz++;
  if (dimension == 2)
    sz = 0;
...
void Neighbor::set(int narg, char **arg, bool auto_set_bin)
{
  ...
  if (style == MULTI && lmp->citeme)
    lmp->citeme->add(cite_neigh_multi);
}
...
void Neighbor::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg)
  {
    ...
    else if (strcmp(arg[iarg], "contact_distance_factor") == 0)
    {
      ...
      if (contactDistanceFactor < 1.0)
        error->all(FLERR, "Illegal neigh_modify command. Please set contact_distance_factor value >=1");
      iarg += 2;
    }
