...
{
  if (domain->lebc)
    std::cout << "neibor_modify every 2 is not working forward_comm\n";
  int n;
  MPI_Request request;
  MPI_Status status;
  ...
  for (int iswap = 0; iswap < nswap; iswap++)
  {
    if (sendproc[iswap] != me)
    {
      ...
      {
        // double send, normal recv
        if (domain->lebc && pbc[iswap][1] != 0 && iswap < 2 * maxneed[1] && (is_bottom_processor || is_top_processor)) //&& dim == 1
        {
          int send_proc_1 = procs[0];
          int send_proc_2 = procs[1];
          int recv_proc_1 = procs[2];
          int recv_proc_2 = procs[3];
          ...
          n = avec->pack_comm_vel(sendnum[iswap], sendlist[iswap],
                                  buf_send, pbc_flag[iswap], pbc[iswap]);
          if (n)
            MPI_Send(buf_send, n, MPI_DOUBLE, send_proc_1, 0, world);
          if (n)
            MPI_Send(buf_send, n, MPI_DOUBLE, send_proc_2, 0, world);
          if (size_forward_recv[iswap])
            MPI_Wait(&request, &status);
          avec->unpack_comm_vel(recvnum[iswap], firstrecv[iswap], buf_recv);
        }
        // normal send, double recv 
        else if (domain->lebc && pbc[iswap][1] == 0 && (is_bottom_processor || is_top_processor) && iswap < 2 * maxneed[1]) // dim == 1
        {
          int send_proc_1 = procs[0];
          int send_proc_2 = procs[1];
          int recv_proc_1 = procs[2];
          ...
          if (recvnum[iswap] * size_forward)
            MPI_Irecv(buf_recv, recvnum[iswap] * size_forward, MPI_DOUBLE,
                      recv_proc_1, 0, world, &request);
          n = avec->pack_comm_vel(sendnum[iswap], sendlist[iswap],
                            buf_send, pbc_flag[iswap], pbc[iswap]);
          if (n)
            MPI_Send(buf_send, n, MPI_DOUBLE, sendproc[iswap], 0, world);
          if (recvnum[iswap] * size_forward)
            MPI_Wait(&request, &status);
          if (doub_recv[iswap] && recv_proc_2_num)
            MPI_Irecv(&buf_recv[recvnum[iswap] * size_forward], recv_proc_2_num * size_forward, MPI_DOUBLE,
                      recv_proc_2, 0, world, &request);
          if (doub_recv[iswap] && recv_proc_2_num)
            MPI_Wait(&request, &status);

          avec->unpack_comm(recvnum[iswap], firstrecv[iswap], buf_recv);
          if (doub_recv[iswap] && recv_proc_2_num)
             avec->unpack_comm_vel(recv_proc_2_num, first_recv_2_num, &buf_recv[recvnum[iswap] * size_forward]);
        } 
        else 
        {
          if (size_forward_recv[iswap])
...
{ 
  ...
  if (domain->lebc) 
  {
    ...
    // if ( tmp2 != atom->natoms)
    // {
    //   std::cout << tmp2 << " " << atom->natoms << "\n";
    // }

    if (atom->firstgroupname)
      atom->first_reorder();

    // std::cout << "end_exchange\n";
  }
  else {
    // std::cout << "begin_exchange\n";
    ...
    for (int dim_o = 0; dim_o < 3; dim_o++)
    {
      ...
      // std::cout << me << " " << update->ntimestep << " " << dim << " "
      //           << "normal stuff\n";
      ...
      //     hi = subhi[dim];
      //     nsend = 0;
      //     // std::cout << me << " " << update->ntimestep << " " << extra.size() << "\n";
      //     for (int ex : extra)
      //     {
      //       // std::cout << me << " " << update->ntimestep << " " << nsend << "\n";
      //       if (nsend > maxsend)
      //         grow_send(nsend, 1);
      //       nsend += avec->pack_exchange_lebc(static_cast<int>(buf[ex]), &buf_send[nsend], &buf[ex]);
      //       // std::cout << me << " " << update->ntimestep << " " << nsend << "\n";
      //     }
      //     extra.clear();
      //     // std::cout << me << " " << update->ntimestep << " "
      //     //           << "b sendrec1\n";
      //     MPI_Sendrecv(&nsend, 1, MPI_INT, procneigh[dim][0], 0,
      //                 &nrecv1, 1, MPI_INT, procneigh[dim][1], 0, world, &status);
      //     // std::cout << me << " " << update->ntimestep << " "
      ...
      //     // std::cout << me << " " << procgrid[dim] << "before_first\n";
      //     MPI_Wait(&request, &status);
      //     // std::cout << me << " " << update->ntimestep << " "
      //     //           << "a pc1\n";
      //     if (z < extra_exchange - 1 || procgrid[dim] % 2 == 1)
      ...
      //                 world, &request);
      //       MPI_Send(buf_send, nsend, MPI_DOUBLE, procneigh[dim][1], 0, world);
      //       // std::cout << me << " " << procgrid[dim] << "before_second\n";
      //       MPI_Wait(&request, &status);
      //       // std::cout << me << " " << update->ntimestep << " "
      //       // << "a pc2\n";
      //     }
...
int Comm::location_to_proc(double *loc)
{
  int i, j, k;

  double len_x = domain->xprd / (float)procgrid[0];
  double len_y = domain->yprd / (float)procgrid[1];
  double len_z = domain->zprd / (float)procgrid[2];

  int pos_x = floor(loc[0] / len_x);
  int pos_y = floor(loc[1] / len_y);
  int pos_z = floor(loc[2] / len_z);

  // std::cout << pos_x << " " << pos_y << " " << pos_z << "\n";

  return grid2proc[pos_x % procgrid[0]][pos_y % procgrid[1]][pos_z % procgrid[2]];
}

void Comm::get_lebc_procs()
{

  double lo = domain->sublo[0];
  double hi = domain->subhi[0];
  double dx = domain->lebc_displacement;

  bool change = false;

  double loc_lo[3];
  double loc_hi[3];
  if (is_bottom_processor)
  {
    lo += dx;
    hi += dx;
    ...
    loc_lo[0] = lo;
    loc_lo[1] = domain->yprd * 0.999;
    loc_lo[2] = (domain->subhi[2] + domain->sublo[2]) / 2;

    loc_hi[0] = hi;
    loc_hi[1] = domain->yprd * 0.999;
    loc_hi[2] = (domain->subhi[2] + domain->sublo[2]) / 2;

    procs[0] = location_to_proc(loc_lo);
    procs[1] = location_to_proc(loc_hi);

    procs[2] = procs[0];
    procs[3] = procs[1];
    // if (procs[0] == procs[1])
    //   std::cout << "error same proc\n";
    ...
    // procs[2] = location_to_proc(loc_lo);
    // procs[3] = location_to_proc(loc_hi);
  ...
  if (is_top_processor)
  {
    ...
    loc_lo[0] = lo;
    loc_lo[1] = domain->yprd * 0.001;
    loc_lo[2] = (domain->subhi[2] + domain->sublo[2]) / 2;

    loc_hi[0] = hi;
    loc_hi[1] = domain->yprd * 0.001;
    loc_hi[2] = (domain->subhi[2] + domain->sublo[2]) / 2;

    procs[0] = location_to_proc(loc_hi);
    procs[1] = location_to_proc(loc_lo);
...
void Comm::borders()
{
  // std::cout << "begin_borders\n";
  int i, n, nn, itype, iswap, dim, ineed, twoneed, smax, rmax;
  ...
  // int proc_1 = -1, proc_2 = -1;
  // double proc_1_lo = 0, proc_2_lo = 0, proc_1_hi = 0, proc_2_hi = 0;
  ...
  for (int dim_o = 0; dim_o < 3; dim_o++)
  {
    ...
    for (ineed = 0; ineed < twoneed; ineed++)
    {
      ...
      if (ghost_velocity)
      {
        // if (domain->lebc && dim == 1 && pbc[iswap][1] != 0)
        // {

        //   n = avec->pack_border_vel_lebc(nsend, sendlist[iswap], buf_send,
        //                                  pbc_flag[iswap], pbc[iswap], dim);

        //   // if (pbc[iswap][1] == 1) // is a bot processor
        //   // {
        //   // }
        //   // if (pbc[iswap][1] == -1) // is a top processor
        //   // {
        //   // }
        // }
        // else
      ...
      if (domain->lebc && dim == 1 && (is_bottom_processor || is_top_processor))
        get_lebc_procs();
      int send_proc_1 = procs[0];
      int send_proc_2 = procs[1];
      ...
      // double send out, but normal recv
      if (domain->lebc && dim == 1 && pbc[iswap][1] != 0)
      {
        ...
        // if (domain->lebc && iswap < 2 * maxneed[1]) //&& dim == 1
        // {
        //   if (iswap == 0)
        //   {
        //     std::cout << me << " " << send_proc_1 << " f " << send_proc_2 << "\n";
        //     std::cout << me << " " << recvproc[iswap] << " fg " << sendnum[iswap] << " " << recvnum[iswap] << " "
        //               << " "
        //               << " " << iswap << "\n";
        //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
        //   }
        // }
        ...
        if (send_proc_2 != me)
        {
          // std::cout << "d right order1\n";

          MPI_Sendrecv(&nsend, 1, MPI_INT, send_proc_1, 0,
                       &nrecv_1, 1, MPI_INT, recvproc[iswap], 0, world, &status);

          nrecv = nrecv_1;
          // if (nrecv == 0)
          // {
          //   std::cout << me << " " << "d nrecv==0\n";
          // }
          if (nrecv_1 * size_border > maxrecv)
            grow_recv(nrecv_1 * size_border);
          if (nrecv_1)
            MPI_Irecv(buf_recv, nrecv_1 * size_border, MPI_DOUBLE,
                      recvproc[iswap], 0, world, &request);
          if (n)
            MPI_Send(buf_send, n, MPI_DOUBLE, send_proc_1, 0, world);
          if (nrecv_1)
        ...
        {
          // std::cout << "d wrong order1\n";
          nrecv = nsend;
          if (nrecv * size_border > maxrecv)
            grow_recv(nrecv * size_border);
          for (int i = 0; i < nrecv * size_border; i++)
          {
        ...
        if (send_proc_2 != me)
        {
          // std::cout << "d right order2\n";

          MPI_Sendrecv(&nsend, 1, MPI_INT, send_proc_2, 0,
                       &nrecv_2, 1, MPI_INT, recv_proc_2, 0, world, &status);
          // nrecv += nrecv_2;
          // recv_proc_2_num = nrecv_2;

          // if (nrecv_2 != 0)
          // {
          //   std::cout << nrecv_2 << " not zero\n";
          // }
          if ((nrecv_1 + nrecv_2) * size_border > maxrecv)
            grow_recv((nrecv_1 + nrecv_2) * size_border);
          if (nrecv_2)
            MPI_Irecv(&buf_recv[nrecv_1 * size_border], nrecv_2 * size_border, MPI_DOUBLE,
                      recv_proc_2, 0, world, &request);
          if (n)
            MPI_Send(buf_send, n, MPI_DOUBLE, send_proc_2, 0, world);
          if (nrecv_2)
        ...
        {
          // std::cout << "d wrong order2\n";
          nrecv += nsend;
          if (nrecv * size_border > maxrecv)
            grow_recv(nrecv * size_border);
          for (int i = nrecv_1; i < nrecv * size_border; i++)
          {
            buf_recv[i] = buf_send[i - nrecv_1];
          }
        }
        buf = buf_recv;
      }
      // normal send, double recv
      else if (domain->lebc && dim == 1 && pbc[iswap][1] == 0 && (is_bottom_processor || is_top_processor))
      {

        // if (iswap == 0)
        // {
        //   std::cout << me << " " << sendproc[iswap] << " sg " << n << "\n";
        //   std::cout << me << " " << recv_proc_1 << " s " << recv_proc_2 << " " << recvnum[iswap] << " "
        //             << " "
        //             << " " << iswap << "\n";
        //   // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
        // }
        ...
        if (sendproc[iswap] != me)
        {
          // std::cout << "s right order1\n";
          MPI_Sendrecv(&nsend, 1, MPI_INT, sendproc[iswap], 0,
                       &nrecv_1, 1, MPI_INT, recv_proc_1, 0, world, &status);
          nrecv = nrecv_1;
          // if (nrecv == 0)
          // {
          //   std::cout << me << " " << "s nrecv==0\n";
          // }
          if (nrecv_1 * size_border > maxrecv)
            grow_recv(nrecv_1 * size_border);
          if (nrecv_1)
        ...
        if (send_proc_2 != me)
        {
          // std::cout << "s right order2\n";
          int temp = 0;

          MPI_Sendrecv(&temp, 1, MPI_INT, send_proc_2, 0,
                       &nrecv_2, 1, MPI_INT, recv_proc_2, 0, world, &status);
          // nrecv += nrecv_2;
          recv_proc_2_num = nrecv_2;
          // if (recv_proc_2_num == 0)
          // {
          //   std::cout << "s recv_proc_2_num==0\n";
          // }
          doub_recv[iswap] = true;
          if ((nrecv_1 + recv_proc_2_num) * size_border > maxrecv)
            grow_recv((nrecv_1 + recv_proc_2_num) * size_border);
          if (nrecv_2)
            MPI_Irecv(&buf_recv[nrecv_1 * size_border], recv_proc_2_num * size_border, MPI_DOUBLE,
                      recv_proc_2, 0, world, &request);
          // if (temp)
          //   MPI_Send(buf_send, temp, MPI_DOUBLE, send_proc_2, 0, world);
          if (nrecv_2)
            MPI_Wait(&request, &status);
        }
        else
        {
          // std::cout << "s wrong order2\n";
          nrecv += nsend;
          if (nrecv * size_border > maxrecv)
            grow_recv(nrecv * size_border);
          for (int i = nrecv_1; i < nrecv * size_border; i++)
      ...
      int global_test;
      // std::cout << "here?\n";
      MPI_Allreduce(&test, &global_test, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      ...
      // std::cout << me << " pre swap " << iswap << "\n";

      if (ghost_velocity)
      {

        avec->unpack_border_vel(nrecv, atom->nlocal + atom->nghost, buf);
        if (iswap < 2 && doub_recv[iswap] && recv_proc_2_num)
        {
          avec->unpack_border_vel(recv_proc_2_num, atom->nlocal + atom->nghost + nrecv, &buf[nrecv * size_border]);
          first_recv_2_num = atom->nlocal + atom->nghost + nrecv;
        }
      }
      else
...
void Comm::forward_comm_fix(Fix *fix)
{
  ...
  // std::cout
  //     << update->ntimestep << " start_forward\n";
  int iswap, n;
  double *buf;
  MPI_Request request;
  MPI_Status status;
  ...
  for (iswap = 0; iswap < nswap; iswap++)
  {
    ...
    if (domain->lebc && pbc[iswap][1] != 0 && iswap < 2 * maxneed[1] && (is_bottom_processor || is_top_processor)) //&& dim == 1
    {
      ...
      // std::cout << me << " fcf double send, normal recv\n";
      ...
      //   if (iswap == 0)
      //   {
      //     std::cout << me << " " << send_proc_1 << " f " << n << "\n";
      //     std::cout << me << " " << recvproc[iswap] << " f " << sendnum[iswap] << " " << recvnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
      ...
      {
        // if (iswap == 1)
        //   std::cout << me << " s " << send_proc_2 << " " << n * sendnum[iswap] << "\n";
        if (recvnum[iswap])
          MPI_Irecv(buf_recv, n * recvnum[iswap], MPI_DOUBLE, recvproc[iswap], 0,
                    world, &request);
        if (sendnum[iswap])
          MPI_Send(buf_send, n * sendnum[iswap], MPI_DOUBLE, send_proc_1, 0, world);
        if (sendnum[iswap])
          MPI_Send(buf_send, n * sendnum[iswap], MPI_DOUBLE, send_proc_2, 0, world);
      ...
      {
        std::cout << me << " fcf d wrong order1\n";
        buf = buf_send;
      }
    }
    // normal send, double recv
    else if (domain->lebc && pbc[iswap][1] == 0 && (is_bottom_processor || is_top_processor) && iswap < 2 * maxneed[1]) // dim == 1
    {
      ...
      // std::cout << me << " fcf s normal send double recv\n";
      ...
      //   if (iswap == 0)
      //   {
      //     std::cout << me << " " << sendproc[iswap] << " s " << n << "\n";
      //     std::cout << me << " " << recv_proc_1 << " s " << sendnum[iswap] << " " << recvnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
      ...
      {
        // if (iswap == 1)
        //   std::cout << me << " r " << recv_proc_2
        //             << " " << n * recv_proc_2_num << "\n";
        if (recvnum[iswap])
          MPI_Irecv(buf_recv, n * recvnum[iswap], MPI_DOUBLE, recv_proc_1, 0,
                    world, &request);
        if (sendnum[iswap])
          MPI_Send(buf_send, n * sendnum[iswap], MPI_DOUBLE, sendproc[iswap], 0, world);
        if (recvnum[iswap])
        ...
        if (iswap < 2 && doub_recv[iswap] && recv_proc_2_num)
          MPI_Irecv(&buf_recv[n * recvnum[iswap]], n * recv_proc_2_num, MPI_DOUBLE, recv_proc_2, 0,
                    world, &request);
        if (iswap < 2 && doub_recv[iswap] && recv_proc_2_num)
          MPI_Wait(&request, &status);
        buf = buf_recv;
      ...
      {
        std::cout << me << " fcf s wrong order1\n";
        buf = buf_send;
      }
    }
    else
    {
      ...
      //   if (iswap == 0)
      //   {
      //     std::cout << me << " " << sendproc[iswap] << " og " << n << "\n";
      //     std::cout << me << " " << recvproc[iswap] << " og " << sendnum[iswap] << " " << recvnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
    ...
    // std::cout << me << iswap << " end of forward\n";
    ...
    if (iswap < 2)
      if (doub_recv[iswap] && recv_proc_2_num)
        fix->unpack_comm(recv_proc_2_num, first_recv_2_num, &buf[n * recvnum[iswap]]);

    // int test = 1;
    // int testr;
    // MPI_Allreduce(&test, &testr, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  }
...
void Comm::reverse_comm_fix(Fix *fix)
{
  ...
  double reverse_doub_recv = false;
  ...
  for (iswap = nswap - 1; iswap >= 0; iswap--)
  {
    ...
    if (iswap < 2)
    {
      if (doub_recv[iswap] && recv_proc_2_num)
      {
        nn = fix->pack_reverse_comm(recv_proc_2_num, first_recv_2_num, buf_send);
        n = fix->pack_reverse_comm(recvnum[iswap], firstrecv[iswap], &buf_send[nn * recv_proc_2_num]);
      }
      else
      {
    ...
    // exchange with another proc
    // if self, set recv buffer to send buffer
    // if (domain->lebc && iswap < 2 * maxneed[1] && (is_bottom_processor || is_top_processor)) //&& dim == 1
    // {
    //   if (iswap == 1)
    //   {
    //     // std::cout << me << " " << n << "\n";
    //     std::cout << me << " " << sendnum[iswap] << " " << recvnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
    //     std::cout << me << " " << sendnum[0] << " " << recvnum[0] << " " << send_proc_1_num[0] << " " << recv_proc_1_num[0] << " " << 0 << "\n";
    //   }
    // }

    // double send, normal recv (but reversed, so one send, two recv)
    if (domain->lebc && pbc[iswap][1] != 0 && iswap < 2 * maxneed[1] && (is_bottom_processor || is_top_processor)) //&& dim == 1
    {
      ...
      //   if (iswap == 1)
      //   {
      //     std::cout << me << " " << recvproc[iswap] << " f " << n << "\n";
      //     std::cout << me << " " << recv_proc_1 << " f " << recvnum[iswap] << " " << sendnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
      ...
      if (sendproc[iswap] != me)
      {
        ...
        reverse_doub_recv = true;

        if (sendnum[iswap])
          MPI_Irecv(buf_recv, n * sendnum[iswap], MPI_DOUBLE, send_proc_1, 0,
                    world, &request);
        if (recvnum[iswap])
          MPI_Send(buf_send, n * recvnum[iswap], MPI_DOUBLE, recvproc[iswap], 0, world);
        if (sendnum[iswap])
        ...
        if (sendnum[iswap])
          MPI_Irecv(&buf_recv[n * sendnum[iswap]], n * sendnum[iswap], MPI_DOUBLE, send_proc_2, 0,
                    world, &request);
        if (sendnum[iswap])
          MPI_Wait(&request, &status);
        buf = buf_recv;
    ...
    else if (domain->lebc && pbc[iswap][1] == 0 && (is_bottom_processor || is_top_processor) && iswap < 2 * maxneed[1]) // dim == 1
    {
      ...
      //   if (iswap == 1)
      //   {
      //     std::cout << me << " " << send_proc_1 << " s " << n << "\n";
      //     std::cout << me << " " << sendproc[iswap] << " s " << recvnum[iswap] << " " << sendnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
      ...
      if (send_proc_1 != me)
      {
        ...
        if (sendnum[iswap])
          MPI_Irecv(buf_recv, n * sendnum[iswap], MPI_DOUBLE, sendproc[iswap], 0,
                    world, &request);
        if (recvnum[iswap])
          MPI_Send(&buf_send[nn * recv_proc_2_num], n * recvnum[iswap], MPI_DOUBLE, recv_proc_1, 0, world);
        if (iswap < 2 && doub_recv[iswap] && recv_proc_2_num)
          MPI_Send(buf_send, nn * recv_proc_2_num, MPI_DOUBLE, recv_proc_2, 0, world);
        if (sendnum[iswap])
          MPI_Wait(&request, &status);
        buf = buf_recv;
      }
    ...
    else
    {
      ...
      //   if (iswap == 1)
      //   {
      //     std::cout << me << " " << recvproc[iswap] << " og " << n << "\n";
      //     std::cout << me << " " << sendproc[iswap] << " og " << recvnum[iswap] << " " << sendnum[iswap] << " " << send_proc_1_num[iswap] << " " << recv_proc_1_num[iswap] << " " << iswap << "\n";
      //     // std::cout << me << " f " << sendnum[1] << " " << recvnum[1] << " " << send_proc_1_num[1] << " " << recv_proc_1_num[1] << " " << 1 << "\n";
      //   }
      // }
    ...
    if (reverse_doub_recv && sendnum[iswap])
    {
      fix->unpack_reverse_comm(sendnum[iswap], sendlist[iswap], &buf[n * sendnum[iswap]]);
      fix->unpack_reverse_comm(sendnum[iswap], sendlist[iswap], buf);
    }
    else if (sendnum[iswap])
    {
      fix->unpack_reverse_comm(sendnum[iswap], sendlist[iswap], buf);
    }

    reverse_doub_recv = false;
  }
  // int test = 1;
  // int testr;
  // MPI_Allreduce(&test, &testr, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
{
  std::cout << "forward_comm_variable_fix\n";
  int iswap, n;
  double *buf;
  MPI_Request request;
  MPI_Status status;
...
{
  std::cout << "reverse_comm_variable_fix\n";
  int iswap, n;
  double *buf;
  MPI_Request request;
  MPI_Status status;
...
{
  if (narg < 1)
    error->all(FLERR, "Illegal communicate command");

  if (strcmp(arg[0], "single") == 0)
    style = SINGLE;
...
void Comm::set_processors(int narg, char **arg)
{
  ...
  if (strcmp(arg[0], "*") == 0)
    ...
    user_procgrid[0] = force->inumeric(FLERR, arg[0]);
  if (strcmp(arg[1], "*") == 0)
    user_procgrid[1] = 0;
  else
    user_procgrid[1] = force->inumeric(FLERR, arg[1]);
  if (strcmp(arg[2], "*") == 0)
    user_procgrid[2] = 0;
  else
    user_procgrid[2] = force->inumeric(FLERR, arg[2]);
  ...
  while (iarg < narg)
  {
    if (strcmp(arg[iarg], "grid") == 0)
    {
      ...
      else if (strcmp(arg[iarg + 1], "twolevel") == 0)
      {
        ...
        if (strcmp(arg[iarg + 3], "*") == 0)
          user_coregrid[0] = 0;
        else
          user_coregrid[0] = force->inumeric(FLERR, arg[iarg + 3]);
        if (strcmp(arg[iarg + 4], "*") == 0)
          user_coregrid[1] = 0;
        else
          user_coregrid[1] = force->inumeric(FLERR, arg[iarg + 4]);
        if (strcmp(arg[iarg + 5], "*") == 0)
          user_coregrid[2] = 0;
        else
          user_coregrid[2] = force->inumeric(FLERR, arg[iarg + 5]);
