...
void Domain::init()
{
  ...
  {
    if (modify->fix[i]->box_change_size)
      box_change_size = 1;
    if (modify->fix[i]->box_change_shape)
      box_change_shape = 1;
    if (modify->fix[i]->box_change_domain)
      box_change_domain = 1;
  }
...
void Domain::update_lebc_displacement()
{
  lebc_displacement += update->dt * ssr * yprd;
  lebc_displacement -= floor(lebc_displacement / xprd) * xprd;
}
...
void Domain::image_check()
{
  ...
  if (!atom->molecular)
    return;
  if (!xperiodic && !yperiodic && (dimension == 2 || !zperiodic))
    return;
  ...
  for (i = 0; i < nlocal; i++)
    for (j = 0; j < num_bond[i]; j++)
    {
      ...
      if (xperiodic && delx > xprd_half)
        flag = 1;
      if (xperiodic && dely > yprd_half)
        flag = 1;
      if (dimension == 3 && zperiodic && delz > zprd_half)
        flag = 1;
      if (!xperiodic && delx > xprd)
        flag = 1;
      if (!yperiodic && dely > yprd)
        flag = 1;
      if (dimension == 3 && !zperiodic && delz > zprd)
        flag = 1;
    }
...
void Domain::box_too_small_check()
{
  ...
  if (atom->nangles)
    maxdelta = 2.0 * maxbondall * BONDSTRETCH;
  if (atom->ndihedrals)
    maxdelta = 3.0 * maxbondall * BONDSTRETCH;

  // warn if maxdelta > than half any periodic box length
  // since atoms in the interaction could rotate into that dimension

  int flag = 0;
  if (xperiodic && maxdelta > xprd_half)
    flag = 1;
  if (yperiodic && maxdelta > yprd_half)
    flag = 1;
  if (dimension == 3 && zperiodic && maxdelta > zprd_half)
    flag = 1;
...
void Domain::remap_lebc(double *x, double *v, tagint &image)
{
  double *lo, *hi, *period, *coord;
  double lamda[3];
  tagint idim, otherdims;
  ...
  if (yperiodic)
  {
    ...
    while (coord[1] < lo[1])
    {
      ...
      idim &= IMGMASK;
      image = otherdims | (idim << IMGBITS);
      coord[0] += lebc_displacement; // TODO?
      v[0] += yprd * ssr;
      if (coord[0] >= xprd)
      {
        coord[0] -= xprd;
    ...
    while (coord[1] >= hi[1])
    {
      ...
      idim &= IMGMASK;
      image = otherdims | (idim << IMGBITS);
      coord[0] -= lebc_displacement; // maybe
      v[0] -= yprd * ssr;            // maybe
      if (coord[0] < 0.0)
      {
        coord[0] += xprd;
...
void Domain::unmap(double *x, tagint image, double *y)
{
  ...
  if (triclinic == 0)
  {
    ...
    y[0] = x[0] + xbox * xprd + ybox * lebc_displacement; // TODO?!
...
void Domain::add_region(int narg, char **arg)
{
  ...
  if (find_region(arg[0]) >= 0)
    error->all(FLERR, "Reuse of region ID");
  ...
  if (!lmp->wb)
  {
    if (strcmp(arg[1], "none") == 0)
      error->all(FLERR, "Invalid region style");
#define REGION_CLASS
...
void Domain::delete_region(int narg, char **arg)
{
  ...
  if (iregion == -1)
    error->all(FLERR, "Delete region ID does not exist");
...
int Domain::find_region(const char *name)
{
  for (int iregion = 0; iregion < nregion; iregion++)
    if (strcmp(name, regions[iregion]->id) == 0)
      return iregion;
  return -1;
}
...
{
  if (narg < 1)
    error->all(FLERR, "Illegal box command");
  ...
  while (iarg < narg)
  {
    if (strcmp(arg[iarg], "tilt") == 0)
    {
      if (iarg + 2 > narg)
        error->all(FLERR, "Illegal box command");
      if (strcmp(arg[iarg + 1], "small") == 0)
        tiltsmall = 1;
      else if (strcmp(arg[iarg + 1], "large") == 0)
        tiltsmall = 0;
      else
        error->all(FLERR, "Illegal box command");
      iarg += 2;
...
void Domain::print_box(const char *str)
{
  if (comm->me == 0)
  {
    if (screen)
    {
      ...
      {
        char *format = (char *)"%striclinic box = (%g %g %g) to (%g %g %g) with tilt (%g %g %g)\n";
