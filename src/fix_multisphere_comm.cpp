...
int FixMultisphere::pack_comm_image_displace(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
    ...
    tagint idim, otherdims, temp;
    // we dont need to account for pbc here

    // LIES!!!! why would you lie to me?!?!? // TODO?
    int yImageChange = -pbc[1];
    ...
    for (i = 0; i < n; i++)
    {
        ...
        // idim = (aimage[j] >> IMGBITS) & IMGMASK;
        // otherdims = aimage[j] ^ (idim << IMGBITS);
        // idim -= 0; // yImageChange     // TODO?!
        // idim &= IMGMASK;
        // temp = otherdims | (idim << IMGBITS);
        ...
        dis_temp[0] = displace_[j][0];
        dis_temp[1] = displace_[j][1];
        dis_temp[2] = displace_[j][2];

        // if (pbc[1] != 0)
        // {
        //     dis_temp[1] += pbc[1] * domain->yprd;
        //     dis_temp[0] += pbc[1] * domain->lebc_displacement;
        // }

        int xbox = (aimage[j] & IMGMASK) - IMGMAX;
        int ybox = (aimage[j] >> IMGBITS & IMGMASK) - IMGMAX;
        int zbox = (aimage[j] >> IMG2BITS) - IMGMAX;

        buf[m++] = static_cast<double>(aimage[j]);
        vectorToBuf3D(dis_temp, buf, m);
    }
    return 4;
}
...
int FixMultisphere::pack_comm_v_omega(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
    ...
    // we dont need to account for pbc here // TODO?
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (pbc[1] != 0)
        {
            // temp_v[0] -= domain->yprd * pbc[1] * domain->ssr;
        }

        vectorToBuf3D(temp_v, buf, m);
...
void FixMultisphere::unpack_comm(int n, int first, double *buf)
{
    if (fw_comm_flag_ == MS_COMM_FW_BODY)
        ...
        error->fix_error(FLERR, this, "FixMultisphere::unpack_comm internal error");
}
...
int FixMultisphere::pack_reverse_comm(int n, int first, double *buf)
{
    ...
    if (rev_comm_flag_ == MS_COMM_REV_X_V_OMEGA)
        ...
        error->fix_error(FLERR, this, "FixMultisphere::pack_reverse_comm internal error");
    return 0;
}
...
int FixMultisphere::pack_reverse_comm_x_v_omega(int n, int first, double *buf)
{
    ...
    for (i = first; i < last; i++)
    {
        ...
        if (tag < 0)
            flag = 0;
        else if (multisphere_.map(tag) >= 0)
            flag = 1;
        else if (corner_ghost[i] == 1.)
            flag = 1;
        else
            flag = 0;
...
int FixMultisphere::pack_reverse_comm_image(int n, int first, double *buf)
{
    ...
    for (i = first; i < last; i++)
    {
        ...
        tag = body_[i];
        if (tag < 0) // not appart of body
            flag = 0;
        else if (multisphere_.map(tag) >= 0) // something?
            flag = 1;
        else if (corner_ghost[i] == 1.) // something else?
            flag = 1;
        else
        {
            flag = 0;
            // flag = 1; // changed to always unpack the image? // TODO?!
            // idim = (temp >> IMGBITS) & IMGMASK;
            // otherdims = temp ^ (idim << IMGBITS);
            // idim -= 1; // yImageChange     // TODO?!
            // idim &= IMGMASK;
            // temp = otherdims | (idim << IMGBITS);
        ...
        // if flag is 1, we unpack the image, if not we do not unpack the image
        buf[m++] = static_cast<double>(flag);
...
void FixMultisphere::unpack_reverse_comm(int n, int *list, double *buf)
{
    if (rev_comm_flag_ == MS_COMM_REV_X_V_OMEGA)
        ...
        error->fix_error(FLERR, this, "FixMultisphere::unpack_reverse_comm internal error");
}
...
void FixMultisphere::unpack_reverse_comm_image(int n, int *list, double *buf)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (flag)
        {
            ...
            int xbox = (image[j] & IMGMASK) - IMGMAX;
            int ybox = (image[j] >> IMGBITS & IMGMASK) - IMGMAX;
            int zbox = (image[j] >> IMG2BITS) - IMGMAX;
...
int FixMultisphere::pack_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
    ...
    if (fw_comm_flag_ == MS_COMM_FW_BODY)
        ...
        error->fix_error(FLERR, this, "FixMultisphere::pack_comm internal error");
    return 0;
}
