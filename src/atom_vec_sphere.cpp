...
void AtomVecSphere::unpack_border_vel(int n, int first, double *buf)
{
  ...
  for (i = first; i < last; i++)
  {
    ...
    // std::cout << x[i][0] << x[i][1] << x[i][2] << tag[i] << type[i] << mask[i] << radius[i];
  }
  if (atom->nextra_border)
    for (int iextra = 0; iextra < atom->nextra_border; iextra++)
      m += modify->fix[atom->extra_border[iextra]]->unpack_border(n, first, &buf[m]);
...
int AtomVecSphere::pack_exchange_lebc(int z, double *buf, double *buf_from)
{
  int count = z;
...
int AtomVecSphere::pack_data_hybrid(int i, double *buf)
{
  ...
  if (radius[i] == 0.0)
    ...
    buf[1] = rmass[i] / (4.0 * MY_PI / 3.0 * radius[i] * radius[i] * radius[i]);
  return 2;
}
...
bigint AtomVecSphere::memory_usage()
{
  ...
  if (atom->memcheck("omega"))
    bytes += memory->usage(omega, nmax, 3);
