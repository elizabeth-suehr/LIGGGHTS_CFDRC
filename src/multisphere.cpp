...
Multisphere::Multisphere(LAMMPS *lmp) : Pointers(lmp),
                           ...
                           double **displace_ins, bool *fflag, bool *tflag, int start_step_ins, double *v_integrate_ins)
{
  ...
  if(std::find(body_types_.begin(), body_types_.end(), type_ins) == body_types_.end()) {
      /* v does not contain x */
    body_types_.push_back(type_ins);
  }
...
void Multisphere::remap_bodies(int *body)
{
  ...
  for (int ibody = 0; ibody < nbody_; ibody++)
  {
    ...
    {
      vcm(velocity, ibody);
      domain->remap_lebc(xbnd, velocity, imagebody_(ibody)); // TODO?
      set_v_body(ibody, velocity);
    }
    else
    {
    ...
    if (original == imagebody_(ibody))
      remapflag_(ibody)[3] = 0;
    else
    {
  ...
  for (int i = 0; i < nlocal + nghost; i++) // TODO?!
  {

    if (body[i] < 0) // does not belong to body
      continue;
    ibody = map(body[i]);

    if (ibody < 0) // body not local?
      continue;
    if (remapflag_(ibody)[3] == 0)
      continue;

    // std::cout << "too late?, particle on boundary?\n";
...
void Multisphere::id_extend_body_extend(int *body)
{
  ...
  {
    if (screen)
      fprintf(screen, "nobody: %d nobody_check: %d, nobody_first: %d. \n", nobody, nobody_check, nobody_first);
...
bool Multisphere::check_lost_atoms(int *body, double *atom_delflag, double *body_existflag, double *volumeweight)
{
  ...
  for (ibody = 0; ibody < nbody_; ibody++)
  {
    if (nrigid_current[ibody] > nrigid_(ibody))
    {
      ...
      error->warning(FLERR, "Internal error in multisphere method more atoms then body needs");
    }
    if (nrigid_current[ibody] != nrigid_(ibody)) // !=
    {
      ...
      deleted_body_types.push_back(type_(ibody));
    }
  }
...
void *Multisphere::extract(const char *name, int &len1, int &len2)
{
  ...
  if (strcmp(name, "nbody") == 0)
    return (void *)&nbody_;
  if (strcmp(name, "nbody_all") == 0)
    return (void *)&nbody_all_;
...
double Multisphere::extract_rke()
{
  ...
  for (int i = 0; i < nbody_; i++)
  {
    ...
    if (inertia_(i)[0] == 0.0)
      ...
      wbody[0] /= inertia_(i)[0];
    if (inertia_(i)[1] == 0.0)
      wbody[1] = 0.0;
    else
      wbody[1] /= inertia_(i)[1];
    if (inertia_(i)[2] == 0.0)
      wbody[2] = 0.0;
    else
      wbody[2] /= inertia_(i)[2];
...
std::vector<int> Multisphere::check_lost_atoms_no_delete(int *body){


  std::vector<int> error_bodies;
  ...
  int track_atom_count_real = 0;
  int track_atom_count_ghost = 0;
  int track_atom_count_ghost_not_first = 0;
  ...
  std::vector<int>lob(comm->nprocs);
  MPI_Allgather(&lostb,1,MPI_INT,&lob[0],1,MPI_INT,world);

  for (int i = 0; i < comm->nprocs; i++)
  {
    if (lob[i] != -1)
    {
      error_bodies.push_back(lob[i]);
    }
  }
  ...
  for (int i = 0; i < nall; i++)
  {
    ...
    if (body_tag >= 0 && map(body_tag) >= 0 && domain->is_owned_or_first_ghost(i))
    {
      if (body_tag == lostb)
      {
        if (i < atom->nlocal)
        {
          track_atom_count_real++;
        }
        if (i >= atom->nlocal)
        {
          track_atom_count_ghost++;
        }
      }
    ...
    if (body_tag >=0 && map(body_tag >=0) && !domain->is_owned_or_first_ghost(i))
    {
      if (body_tag == lostb)
      {
        if (i >= atom->nlocal)
        {
          track_atom_count_ghost_not_first++;
        }
      }
    }
  }
  ...
  for (int ibody = 0; ibody < nbody_; ibody++)
  {
    if (nrigid_current[ibody] > nrigid_(ibody))
    {
      ...
      error->warning(FLERR, "Internal error in multisphere method more atoms then body needs");
      std::cout << tag(ibody) << " extra atoms real: " << track_atom_count_real << " ghost: " << track_atom_count_ghost << " not_first: " << track_atom_count_ghost_not_first << "\n";

    }
    if (nrigid_current[ibody] < nrigid_(ibody))
    {
        error->warning(FLERR, "Internal error in multisphere method less atoms then body needs");
        std::cout << tag(ibody) << " missing atoms real: " << track_atom_count_real << " ghost: " << track_atom_count_ghost << " not_first: " << track_atom_count_ghost_not_first << "\n";
    }
  }
