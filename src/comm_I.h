...
inline bool Comm::decide(int i, int dim, double lo, double hi, int ineed)
{
    ...
    // if (domain->lebc && dim == 0 && i > atom->nlocal)
    // {
    //     int twoneed_y = 2 * maxneed[0];
    //     int iswap = twoneed_y + ineed;

    //     if (is_top_processor)
    //     {
    //         if (x[i][1] > domain->boxhi[1])
    //         {
    //             return false;
    //         }
    //     }
...
inline bool Comm::decide_lebc(double *buf, int dim, double lo, double hi, int ineed)
{
    double x[3] = {buf[0], buf[1], buf[2]};
    double radius = buf[6];

    if (((ineed % 2 == 0) && x[dim] >= lo && x[dim] <= (hi + (use_gran_opt() ? (radius) : 0.))) ||
        ((ineed % 2 == 1) && x[dim] >= (lo - (use_gran_opt() ? radius : 0.)) && x[dim] <= hi))
        return true;
...
void Comm::exchangeEventsCorrector()
{
    ...
    for (int iPro = 1; iPro < nprocs; iPro++)
        exchangeReceiveDisplacement[iPro] = exchangeReceiveCounts[iPro - 1] + exchangeReceiveDisplacement[iPro - 1];
